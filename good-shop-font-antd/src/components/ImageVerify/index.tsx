...
export const ImageVerify = ({ onSubmit }: ImageVerifyProps) => {
  const verifyRef: any = useRef(null);
  ...
  const draw = () => {
    ...
    let imgCode = '';
    // 3.填充背景颜色，背景颜色要浅一点
    const ctx = verifyRef.current.getContext('2d');
    // 填充颜色
    ctx.fillStyle = randomColor(180, 230);
    // 填充的位置
    ctx.fillRect(0, 0, data.width * 2, data.height * 2);
    ...
    for (let i = 0; i < 4; i++) {
      ...
      imgCode += text;
      // 随机的字体大小
      const fontSize = randomNum(50, 50);
      // 字体随机的旋转角度
  ...
  return (
    <div style={{ lineHeight: 0 }}>
      <canvas
        style={{ width: data.width, height: data.height, cursor: 'pointer' }}
        onClick={() => handleDraw()}
        ref={verifyRef}
      ></canvas>
    </div>
